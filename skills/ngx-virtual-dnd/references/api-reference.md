...
| `dragHandle` | `string` | `undefined` | No | CSS selector restricting drag initiation area; pressing the handle starts a drag even when it is a control (controls inside it keep their presses). Outside it, pointer-downs inside controls (`button`, `input`, `textarea`, `select`, `summary`, `[contenteditable]`, ARIA widgets, controls in a web component's open shadow DOM) or directly on a `.no-drag` element never start a drag |
