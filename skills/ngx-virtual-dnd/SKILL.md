...
- **`dragHandle`** — CSS selector; only pointer-downs inside a matching element start a drag. The handle may itself be a control (a `<button>`, `role="button"`, or a web component such as `ion-button`): pressing it, or `Space` on it, starts a drag. Controls inside the handle keep their presses.
- **Interactive children** — pointer-downs inside a control never start a drag, so controls inside items keep working. Controls are `button`, `input`, `textarea`, `select`, `summary` and `[contenteditable]` elements, elements with an ARIA role `button`, `checkbox`, `combobox`, `radio`, `searchbox`, `slider`, `spinbutton`, `switch` or `textbox`, and controls inside a web component's open shadow DOM (e.g. `ion-toggle`, `ion-select`, Shoelace inputs). The draggable element itself, anything it renders in its own shadow DOM, and elements around it never count (`<button vdndDraggable>` drags). The same applies to an element with class `no-drag`, but only when it is the exact element pressed (its children are not covered): use it for controls that can't be recognized (a clickable element without a role, a web component with a closed shadow root; put it on the component element). Only the primary mouse button starts a drag. `Space` pressed in such a control (or on a `no-drag` element) inside an item goes to the control instead of starting a keyboard drag. The `dragHandle` is the exception: pressing it or `Space` on it starts a drag, even when it is a control. If an item's content is itself a control (e.g. `<ion-item button>`), put `vdndDraggable` on that element or add a drag handle.
...
| A custom control inside an item starts a drag instead of working | Native controls, ARIA widgets and controls in open shadow DOM are excluded already; for others add class `no-drag` to the element that receives the press, or use `dragHandle` |
| Pressing an item's content never starts a drag | The content is itself a control (e.g. `<ion-item button>`): put `vdndDraggable` on it or add a drag handle |
